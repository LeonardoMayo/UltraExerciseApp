...
public class FileToTileList implements ObservableList {

  public FileToTileList(){

  }

  public void addItemArraylist(ArrayList list){
  ...
  /**
   * Tries to removed a listener from this observable list. If the listener is not attached to this
  ...
  /**
   * Basically a shortcut to sublist(from, to).clear() As this is a common operation, ObservableList
   * has this method for convenient usage.
   *
   ...
   * pairs of elements in the two lists are <i>equal</i>.  (Two elements <tt>e1</tt> and <tt>e2</tt>
   * are <i>equal</i> if <tt>(e1==null ? e2==null : e1.equals(e2))</tt>.)  In other words, two lists
   ...
   * }</pre>
   * This ensures that <tt>list1.equals(list2)</tt> implies that <tt>list1.hashCode()==list2.hashCode()</tt>
   * for any two lists, <tt>list1</tt> and <tt>list2</tt>, as required by the general contract of
   ...
   * algorithms in the <tt>Collections</tt> class can be applied to a subList.<p>
   *
   ...
   * than once. That is, no check is made to ensure uniqueness. <p> Note that the same actual {@code
   * InvalidationListener} instance may be safely registered for different {@code Observables}. <p>
   ...
   * the {@code Observable} becomes invalid. <p> If the given listener has not been previously
   * registered (i.e. it was never added) then this method call is a no-op. If it had been
   * previously added then it will be removed. If it had been added more than once, then only the
